...
class SocketHandler:
    ...
    def listen(self):
        ...
        try:
            while True:
                ...
                if req.request_id == 0:
                    # No request_id => push from server
                    self.push_queue.put(req)
                else:
                    # A waiting thread is expecting this response
    ...
    def send(
        ...
        the operation and data to be transmitted. Automatically
        generates a `request_id` to be used to identify the response.
        """
...
class Client:
    def __init__(self, master, sock: socket.socket):
        ...
        self.chat_treeview = ttk.Treeview(
            self.messages_frame, columns=("id", "message"), show="headings"
        )
        ...
        self.prev_accounts_button = tk.Button(
            self.accounts_pagination_frame, text="Previous", command=self.prev_accounts_page
        )
    ...
    def load_previous_messages_page(self, page: int = 1, per_page: int = 10):
        """
        This loads previously read messages. By server design, these
        are newest->oldest. We'll unify them in the local store by
        actual timestamp or ID.
        """
        ...
        if response.request_code == REQUEST_SUCCESS_CODE:
            ...
            if messages:
                self.update_message_store(messages)
                self.refresh_chat_view()
            # If you have pagination logic, you'd track if there's another page
        else:
            messagebox.showerror(
                f"Error: {response.request_code}",
                response.data.get("error", "Unknown error."),
    ...
    def insert_message_into_treeview(self, msg):
        ...
        if msg["from"] == self.current_user:
            display_text = (
                f"You to {msg['to']} ({msg['timestamp']}): {msg['content']}"
            )
    ...
    def on_scroll(self, first, last):
        """
        If you want "infinite scrolling" for older messages, detect 
        if the user scrolls near bottom or top. Not strictly needed
        if you're just loading a single page of read messages.
        """
        pass
    ...
    def load_accounts_page(self, page: int, per_page: int = 10):
        req_data = {
