...
class BVCodec(Codec):
    ...
    def _encode_datetime(self, obj: datetime) -> bytes:
        type_code = self.TYPE_DATETIME.to_bytes(1, self.byteorder)
        value = int(obj.timestamp() * 1000000).to_bytes(8, self.byteorder, signed=True)
        length = len(value).to_bytes(2, self.byteorder)
        return type_code + length + value
    ...
    def decode(self, data: bytes) -> tuple[Any, bytes]:
        ...
        elif type_code == self.TYPE_DATETIME:
            assert len(remaining) >= length
